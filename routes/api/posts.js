...
router.delete("/:id", auth, async (req,res) => {
    try {
        ...
        if(post.user.toString() !== req.user.id) {
            return res.status(401).json({ msg: "post not authorized to be deleted" })
        }
...
// @desc    Delete commented post by id
// @access  Private
router.delete("/comment/:id/:comment_id", auth, async (req,res) => { 
    try {
        ...
        removeCommentIndex = post.comments.map(comment => comment.id).indexOf(req.params.comment_id);
